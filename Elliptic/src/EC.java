...
// Completed by Kristian Snyder 2/21/17
...
Point multiply(Point P, BigInteger n){
 ...
 void checkParameters(){
   ...
   System.out.println("The following two lines should be the same:");
 ...
 void sharedSecret(){  // Figure 10.7 of book
    ...
    Point KB = multiply(publicKeyA, privateKeyB);// secret computed by B
     System.out.println("Shared secrets (should be the same):");
 ...
 void encryptionForB(){  // page 305 of book
   ...
   System.out.println(message.toString(16) + "\n");
    //int baseParam = 20; // for use in Koblitz encoding
   BigInteger k = new BigInteger(privateKeySize, random);
   Point kG = multiply(G, k); // k times G
   Point kY = multiply(publicKeyB, k); // k times publicKeyB (kPb)
