...
// based on suggestion of Jonathan Eichison
        return len;
    }
    ...
    void addRoundKey1(int round){
        for (int k = 0; k < blockSize; k++)
            state[k] ^= roundKey1[round][k];
    }
    ...
    void xex(){  // XTS-AES block encryption
        addBlock(state, T);
        blockCipher1();
        addBlock(state, T);
    }
    ...
    public static void main(String[] args){
        if (args.length < 2){
            System.err.println("Usage: java DE9eXTS doublekey tweak < original > DE9testXTS.de9");
            return;
        }
        DE9eXTS de9 = new DE9eXTS();
        de9.makeLog();
        de9.buildS();
        de9.readKey(args[0]);
        de9.expandKey();
